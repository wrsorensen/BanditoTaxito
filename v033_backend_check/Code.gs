/**
 * v0.3.3 — Expanded Receipt Financial Fields API
...
function analyzeReceiptDraft(payload) {
  ...
  try {
    const draft = callGeminiReceiptDraft_(payload.file || {});
...
function callGeminiReceiptDraft_(filePayload) {
  ...
  const prepared = validateReceiptAiFile_(filePayload);
  const endpoint = 'https://generativelanguage.googleapis.com/v1beta/models/' + encodeURIComponent(model) + ':generateContent';
  const prompt = [
    'Read this receipt or invoice image and return one JSON object only.',
  ...
  const modelText = Array.isArray(parts) ? parts.filter(function(part) {
